...
// the raytracer for cs430
// made by jack normand and includes reflections and everything else
...
int main();
void getColor(float* dest, ray inputRay, object sceneObjects[], int depth, int colorFlag);
...
// finds intersection between sphere or plane
float intersectObj(object* objectShotAt, ray inputRay)
{
...
// finds the final pixel color, factors in light
void getLitColor(float *dst, ray inputRay, object* currentObj, object* currentLight, float tVal, object sceneObjects[])
{
...
// recursive function to get the final color of the pixel to return to main loop
void getColor(float* dest, ray inputRay, object sceneObjects[], int depth, int colorFlag)
{
    ...
    // loop through objects and get nearest tval, nearest obj
    ...
    if(nearestObj)
    {
        ...
        for (lightIndex = 0; lightIndex < 128; lightIndex++)
        {
            ...
            if (sceneObjects[lightIndex].kind == LIGHT)
            {
                    ...
                    // get relfection color
                    if (nearestObj->kind == SPHERE)
                    {
                        // get reflected ray
                        ray reflectedSphereRay;
                        
                        float hitpoint[] = {0,0,0};
                        float directionTimesLen[] = {inputRay.direction[0],inputRay.direction[1],inputRay.direction[2]};
                        float sphereNormal[] = {0,0,0};
                        float reflectDir[] = {0,0,0};
                        ...
                        float reflectOrigin[] = {hitpoint[0], hitpoint[1], hitpoint[2]};
                        v3_subtract(sphereNormal, hitpoint, nearestObj->properties.sphere.position);
                        v3_normalize(sphereNormal, sphereNormal);


                        // reflect
                        v3_reflect(reflectDir, inputRay.direction, sphereNormal);


                        // set new ray values
                        reflectedSphereRay.direction = reflectDir;
                        reflectedSphereRay.origin = reflectOrigin;

                        getColor(reflectCol, reflectedSphereRay, sceneObjects, depth + 1, colorFlag);

                        v3_scale(reflectCol, nearestObj->properties.sphere.reflectivity);

                        v3_add(pixelColor, pixelColor, reflectCol);

                    }
                    else if (nearestObj->kind == PLANE)
                    {
                        // get reflected ray
                        ray reflectedPlaneRay;
                        ...
                        // reflect
                        v3_reflect(reflectDir, inputRay.direction, nearestObj->properties.plane.normal);

                        // set new ray values
                        reflectedPlaneRay.direction = reflectDir;
                        reflectedPlaneRay.origin = reflectOrigin;

                        getColor(reflectCol, reflectedPlaneRay, sceneObjects, depth + 1, colorFlag);

                        v3_scale(reflectCol, nearestObj->properties.plane.reflectivity);
...
// main function. includes parser and main raycaster loop
int main(int argc, char *argv[])
{
    ...
    // BEGIN LOOP FOR RED AND BLUE
    int counter = 0;
    int maxCount = 2;
    int redFlag = 1;
    int redImage[sceneWidth*sceneHeight][3];
    int blueImage[sceneWidth*sceneHeight][3];
    // cam pos
    float camPosX = 0;
    ...
    for ( heightIndex = sceneHeight-1; heightIndex >= 0; heightIndex--)
    {
        ...
        for (widthIndex = 0; widthIndex < sceneWidth; widthIndex++)
        {
            ...
            {
            // get ray color 
            getColor(pixelColor, currentRay, sceneObjects, 0, redFlag); 

            int red = pixelColor[0]*255;
            int green = pixelColor[1]*255;
            int blue = pixelColor[2]*255;

            redImage[totalIndex][0] = red;
            redImage[totalIndex][1] = green;
            redImage[totalIndex][2] = blue;
            }
            else if(redFlag == 0)
            {
            // get ray color 
            ...
            blueImage[totalIndex][0] = red;
            blueImage[totalIndex][1] = green;
            blueImage[totalIndex][2] = blue;
            }
    ...
    // increment counter so we know to stop loop 
    counter++;
    // increment cam position for blue image
    camPosX+=.5;
    // turn redflag to 0 so we know to do blue image now
    redFlag = 0;
    }
    ...
    // combine blue and red images to make a 3d image
    for ( int pixelIndex=0; pixelIndex < sceneHeight*sceneWidth; pixelIndex++)
    {
        fprintf(outputFile, "%d %d %d ", redImage[pixelIndex][0]+ blueImage[pixelIndex][0],
                                            redImage[pixelIndex][1]+ blueImage[pixelIndex][1],
                                            redImage[pixelIndex][2]+ blueImage[pixelIndex][2]);
    }
